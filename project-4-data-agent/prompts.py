...
def get_agent_descriptions() -> Dict[str, Dict[str, Any]]:
    ...
    return {
        "web_researcher": {
            "name": "Web Researcher",
            "capability": "Fetch public data via Tavily Web search API.",
        ...
        "cortext_researcher": {
            ...
            "capability": "Query private/company data in Snowflake, including structured deal records (company name, deal value" + 
            "sales rep, close date, deal status, product line)) and unstructured sales meeting notes, via Snowflake Cortex Agents.",
            ...
            "output_format": "For structured requests, return the exact fields and include SQL when applicable; for" + 
            "unstructured, return concise relevant excerpts with citations."
        },
        "chart_generator": {
    ...
    "synthesizer": {
        ...
        "output_format": "Coherent written summary incorporating all research findings",
...
def _get_enabled_agents(state: State| None = None) -> List[str]:
    ...
    baseline = ["web_researcher", "chart_generator", "synthesizer", "chart_summarizer"]
    if not state:
        return baseline
    val = state.get("enabled_agents") if hasattr(state, "get") else getattr(state, "enabled_agents", None)
    
    if isinstance(val, list) and val:
        allowed = {"web_researcher", "cortext_researcher", "chart_generator", "chart_summarizer", "synthesizer"}
...
def plan_prompt(state: State) -> HumanMessage:
    ...
    enabled_for_planner = [
        a for a in enabled_list if a in ("web_researcher", "cortext_researcher", "chart_generator", "synthesizer")
    ]
    
    planner_agent_enum = " | ".join(enabled_for_planner) or "web researcher | chart generator | synthesizer"
    ...
    For example, if the user's query is "What were the key action items in the last quarter, and what a recent news story for each of them?", you
    ...
    4. Fetch a recent news story for the third action item.
